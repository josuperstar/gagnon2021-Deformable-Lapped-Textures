...
//------------------------------------------------------------------------------
template<class T>
void SpatialGrid<T>::initGrid(double cellSize, const Vec3f& volumeMin, const Vec3f& volumeMax)
{
	// Clear previous grid
	if (!_grid.empty())
	{
...
template<class T>
inline
bool SpatialGrid<T>::insert(const T& element, const Vec3f& position)
{
	// Get Index
...
template<class T>
inline
bool SpatialGrid<T>::insert(const T& element, const Vec3f& AABBmin, const Vec3f& AABBmax)
{
	// Get min/max indices
...
template<class T>
inline
void SpatialGrid<T>::getElements(const Vec3f&	position,
								 double 			radius,
		...
		for (int iy=yMin; iy<=yMax; ++iy)
		{
			for (int iz=zMin; iz<=zMax; ++iz)
			{
				...
				// Traverse elements in cell
                                std::vector<T>& cell = _grid.at(cellIndex);
        ...
        for (int ix=xMin; ix<=xMax; ++ix)
        {
                for (int iy=yMin; iy<=yMax; ++iy)
                {
                        for (int iz=zMin; iz<=zMax; ++iz)
                        {
                                ...
                                // Traverse elements in cell
                                const std::vector<T>& cell = _grid.at(cellIndex);
                                for (int i=0; i<cell.size(); ++i, ++e)
                                {
                                        if (e < elements.size())
                                        {
...
template<class T>
inline
int SpatialGrid<T>::getXIndex(const Vec3f& position) const
{
...
template<class T>
inline
int SpatialGrid<T>::getYIndex(const Vec3f& position) const
{
...
template<class T>
inline
int SpatialGrid<T>::getZIndex(const Vec3f& position) const
{
