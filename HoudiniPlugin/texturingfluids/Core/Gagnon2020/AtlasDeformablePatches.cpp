...
AtlasDeformablePatches::~AtlasDeformablePatches()
{
    cout <<"Destroying AtlasDeformablePatches"<<endl;
...
bool AtlasDeformablePatches::BuildAtlas(int w, int h, int life)
{
...
void AtlasDeformablePatches::CreateListGUDetails()
{
    GA_PrimitiveGroup *primGroup;
...
void AtlasDeformablePatches::RasterizePrimitive(GA_Offset primOffset, int w, int h,ParametersDeformablePatches params)
{
    ...
    for(int i =min.x()-pixelCellSize; i < max.x()+pixelCellSize; i++)
    {
        for(int j =min.y()-pixelCellSize; j < max.y()+pixelCellSize; j++)
        {
            ...
            if (inTriangle && notUsedYet)
            {
                ...
                color.B = 1;
                //======================== Yu2011  function =====================
                Pixel R_eq4 = PixelColorComputation::Blend(deformableGrids,i,j,w,h,
                ...
                //======================== End Test encapsulated function =====================
                    ...
                    displacementMapEquation4->SetColor(pixelPositionX,h-pixelPositionY,0,displacementSumEq3);
...
void AtlasDeformablePatches::SaveAtlas()
{
...
void AtlasDeformablePatches::BoundingBox2D(UT_Vector3 a, UT_Vector3 b, UT_Vector3 c,UT_Vector3 &min,UT_Vector3 &max)
{
    min.x() = a.x();
    min.y() = a.y();
    //min.z = a.z;
...
bool AtlasDeformablePatches::IsPointInTriangle(UT_Vector3  p, UT_Vector3 a,UT_Vector3 b,UT_Vector3 c)
{
    UT_Vector3 v0 = c - a;
    UT_Vector3 v1 = b - a;
    UT_Vector3 v2 = p - a;
...
Pixel AtlasDeformablePatches::SetRandomColor(int patchNumber)
{
...
void AtlasDeformablePatches::initPatchColors(GU_Detail *trackersGdp)
{
